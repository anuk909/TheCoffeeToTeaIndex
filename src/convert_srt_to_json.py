...
def convert_srt_to_json(srt_file_path, json_file_path):
    ...
    if not subs:
        logging.error(f"Error: SRT file '{srt_file_path}' is empty or not properly formatted.")
        return
    ...
    try:
        ...
        logging.info(f'Subtitles have been successfully converted to JSON and saved to {json_file_path}')
    except IOError as e:
        logging.error(f"Error: Unable to write to JSON file '{json_file_path}'. {str(e)}")
        return

    # Confirm successful writing of the output JSON file
    if os.path.isfile(json_file_path) and os.path.getsize(json_file_path) > 0:
        logging.info(f"Output JSON file '{json_file_path}' has been successfully written and verified.")
    else:
        logging.error(f"Error: Output JSON file '{json_file_path}' was not written successfully.")
